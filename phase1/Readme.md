...
PySpark: customers.show()

3. Show customers from Chennai
SQL: SELECT * FROM customers WHERE city = 'Chennai';
PySpark: customers.filter(customers.city == "Chennai").show()

5. Show customers with age > 25
SQL: SELECT * FROM customers WHERE age > 25;
PySpark: customers.filter(customers.age > 25).show()

7. Show only customer_name and city
SQL: SELECT customer_name, city FROM customers;
PySpark: customers.select("customer_name", "city").show()

9. Count customers city-wise
SQL:
SELECT city, COUNT(*) 
FROM customers 
GROUP BY city;
